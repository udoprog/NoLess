...
class NFileStream {
	...
	public function open( $path , $mode , $extendMode = false ) {
		...
		if ( !$path ) throw new Exception( "File not found" );
		
		if ( !$this->FileHandle = fopen( $path , $mode ) ) throw new Exception( self::ERROR_FAILED_OPEN_STREAM );
	...
	public function readDocument( $doc ) {
		if ( !$this->isOpen() ) throw new Exception( self::ERROR_STREAM_NOT_OPEN );
		$this->write( $doc->getOutput() );
		return false;
	}
