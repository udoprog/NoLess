	...
	public function post( $post ) {
		...
		switch ( $this->action ) {
			case "create_thread":
					...
					if ( !isset( $this->params[ 'parent' ] ) ) $this->errors[] =		"'parent' not specified";
		...
		if ( count( $this->errors ) ) {
			...
			switch ( $this->action ) {
				case "list":
					...
					switch ( $this->type ) {
						case "threads":
							$qry = "SELECT * FROM forum_threads WHERE parent_name = '" . $this->params[ 'parent' ] . "'";
							...
							foreach ( $threads as $thread ) {
								...
								$list->appendChild( new DOMElement( "author_id" ) )->appendChild( new DOMText( $thread['author_id'] ) );
								$list->appendChild( new DOMElement( "parent_name" ) )->appendChild( new DOMText( $thread['parent_name'] ) );
								$list->appendChild( new DOMElement( "parent_tree" ) )->appendChild( new DOMText( $thread['parent_tree'] ) );
						...
						case "entries":
							if ( !isset( $this->params[ 'thread' ] ) ) $this->errors[] =		"'thread' not specified";
							if ( count( $this->errors ) ) $this->dump();

							$qry = "SELECT * FROM forum_entries WHERE thread_name = '" . $this->params[ 'thread' ] . "'";
			...
			case "create_thread":
					...
					try {
						...
						$thread = $this->sql->query( "SELECT name FROM forum_threads WHERE name = '" . $this->params[ 'name' ] . "';" )->getArray();

						if ( isset( $thread[0]['name'] ) ) {
							$this->errors[] = "Thread already exists!";
							$this->dump();
						}
						
						$parent_thread = $this->sql->query(
							"SELECT parent_tree FROM forum_threads WHERE name = '" . $this->params[ 'parent' ] . "';"
						)->getArray();
						
						if ( !isset( $parent_thread[0] ) ) {
							$parent_tree = "/index/";
						} else {
							$parent_tree = $parent_thread[0]['parent_tree'] . $this->params[ 'parent' ] . "/";
						}
						
						$threads = $this->sql->query(
							"SELECT name , parent_tree FROM forum_threads WHERE name = '" . $this->params[ 'name' ] . "';"
						)->getArray();
						
						foreach( $threads as $thread ) {
							if ( strtolower( $thread[ 'parent_tree' ] . $thread[ 'name' ] ) == strtolower( $parent_tree ) )
							{
								$this->errors[] = "Thread already exists";
								$this->dump();
							}
						...
						$this->sql->insert(
							...
							array(
								...
								"parent_id" => 1,
								"parent_name" => $this->params[ 'parent' ],
					...
					try {
						...
						$this->sql->insert(
							...
							array(
								...
								"thread_id" => 1,
								"thread_name" => $this->params[ 'thread' ],
			...
			case "remove_thread":		
					try {
						$user = $this->nsecure->db_GetUser( $this->sql );
						$entry = $this->sql->query(
							"SELECT owner_name , name FROM forum_threads WHERE id = " . $this->params[ 'id' ] . ";"
						)->getArray();
						...
						//$this->sql->query("DELETE FROM forum_entries WHERE thread_name ~* '" . $entry['name'] . "';");
